...
arr[index].to_string()`,
    ...
    let _s2: String = "hello RareCode".into();`,
      ...
      { type: "subtitle", content: "Mutating Strings (push, push_str)" },
      {
        type: "code",
        code: `s.push('c') // single character to mut String, here s needs to be String
...
TokenAccount {
    ...
    data: {
      ...
      {
        type: "subtitle",
        content: "Implimentation of assigning a MetaPleex's URI using trait",
      },
      {
        type: "code",
    ...
    sections: [
      {
        type: "note",
        content:
          "If an integer cast always succeeds (i.e. we are casting a smaller integer to a larger one), we can use the .into() or TYPE::from syntax.",
      },
      { type: "subtitle", content: "Basics" },
      {
        type: "code",
    ...
    sections: [
      {
        type: "note",
        content:
          ".try_into() returns a Result containing either the outcome of the cast, or an error describing why the cast failed.",
      },
    ...
    sections: [
      {
        type: "note",
        content:
          "Remember, keys in a HashMap must remain immutable after insertion. Since modifying a key would change its hash and potentially collide with existing keys, you must create a new HashMap and reassign entries instead of mutating keys directly.",
      },
    ...
    sections: [
      {
        type: "note",
        content:
          "Rust does not allow floats to be combined with integers in arithmetic operations. They are copy types so does not move on assignment to other var.",
      },
...
let my_range: Range<i32> = 0..10;`,
      ...
      {
        type: "subtitle",
        content: "Advanced Example: Sum of Previous Elements",
      },
      {
        type: "code",
                ...
                <div className="card-icon">
                  {topic.title.toLowerCase().includes("ownership")
                    ...
                    : topic.title.toLowerCase().includes("hash") &&
                      !topic.title.toLowerCase().includes("hashing")
                    ? "🔑"
                    ...
                    ? "🛟"
                    : topic.title.toLowerCase().includes("try")
                    ? "⚖️"
                    : topic.title.toLowerCase().includes("result")
                    ? "✅"
                    : topic.title.toLowerCase().includes("from")
                    ? "➡️"
                    ...
                    ? "🧩"
                    : topic.title.toLowerCase().includes("implimentation")
                    ? "⚙️"
