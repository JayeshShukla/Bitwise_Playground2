...
pub fn parse_or_remove(v: &[&str]) -> Vec<i16> {
  ...
  // =========================================================
  {
    ...
    sections: [
      { type: "note", content: "Like enums, without the Copy attribute, structs cannot be copied." },
...
sl.iter().enumerate().map(|(x,&y)| {x as i32 +y}).collect() // due to above reason &y will come`
  ...
  // =========================================================
  {
    ...
    sections: [
      { type: "note", content: "In Rust, the .filter() method passes a reference to the element into the closure." },
